...
:
    ...
    void setSnapshotRate(real interval)
	{
	    ...
	    {
		snapshots = false;
		snapshotTimeInterval = endTime - startTime + 1.0f;	// useles as the flag is false, yet another way of having no snapshots
	    }
	};
    ...
    // WAVESPEEDS
    // There is no need to have accessor method for the wave speed buufers as
    // the host never need to access the data in these buffers.
    // When the buffers are created we can overwrite the junk data they contain
    // by simply copying zero onto the entire buffer.
};
